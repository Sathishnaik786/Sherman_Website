...
const RESPONSES: Record<string, string> = {
  ...
  products: "Our proprietary solutions include Farm2Plate (Agri-traceability), Smart Meter (Utility Intel), TAMRA (Mining Governance), and Infracon. Any specific one you're interested in?",
  contact: "You can reach our experts via the 'Get Quote' button, call us at +91-8851599751, or use the WhatsApp button on the right. Should I provide more details?",
...
export function useChat() {
  ...
  const getBotResponse = (input: string): string => {
    ...
    if (lowercaseInput.includes('about') || lowercaseInput.includes('who are you')) return RESPONSES.about;
